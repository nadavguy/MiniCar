...
class Kalman {
  ...
  public:
    ...
    void ReInit(double process_noise, double sensor_noise, double estimated_error, double intial_value) 
    {
        this->q = process_noise;
        this->r = sensor_noise;
        this->p = estimated_error;
...
//pEstimatedError = ЕСЛИ(ABS( AxEstimation - AxRaw )> ( FilterResetThreshold );( initialErrorP0 );( 1 - Kalmangain ) * EstimatedErrorPrev)
//
//Kalmangain = ( EstimatedErrorPrev + qProcessNoise ) / ( (pEstimatedError + qProcessNoise + rSensorNoise )
//
//AxEstimation = (AxEstimationPrev) + ( Kalmangain ) * (AxRaw - (AxEstimationPrev)
//
//
//
//
...
//
//
//      pEstimatedError = pEstimatedError + qProcessNoise;
//    
//      //measurement update
//      kKalmangain = pEstimatedError  / (pEstimatedError + rSensorNoise );
//      
//      xEstimation  = xEstimation  + kKalmangain * ( AxRaw - xEstimation );
//      pEstimatedError = (1 - kKalmangain) * pEstimatedError;
//      
//      return xEstimation;
    ...
    void setParameters( double sensor_noise) {
        this->r = sensor_noise;
    }
